...
'''
Script para generar csv de scores (RMSE; MAE; Bias y R) a partir de datos de obs
(del KNMI) y modelo WRF.

----------------------------------------------------------------------------------
Toma funciones de otros scrips del proyecto, que son los siguientes:

- cargar_datos (procede de import_KNMI_data): genera un df con datetime index 
y cada columna representa una de las estaciones de la red del KNMI

- extract_point_data (procede de import_wrfout_data): extrae los datos de un 
wrfout correspondientes a un punto lon,lat.

- process_wrf_file (procede de import_wrfout_data):
----------------------------------------------------------------------------------

----------------------------------------------------------------------------------
CONSIDERACIONES:
- Si la variable compute_data == True, el código genera las tablas de scores (util 
si no están generadas previamente o si las cambiar). Si compute_data = False, las 
lee en el directorio: Projects/SB_SCALING-Netherlands/misc/WRF_validation_csv_files

- Si avg_zones == True, los scores son promediados para cada region 'LOC', dando 
lugar a un valor promediado de cada score para cada LOC. Siendo el csv generado 
de este estilo:
...
Si avg_zones == False, en la tabla aparecera cada una de las estaciones y sus scores
 correspondientes. Siendo el csv generado de este estilo:
...
estadisticos_sims = [] # se rellenará con los df de estadisticos para cada simulacion
...
def apply_colored_styles(df, var_units):
...
for sim_name in sim_names:
    ...
    for period_computation in periods_computation:
        ...
        apply_colored_styles(estadisticos, var_units)
