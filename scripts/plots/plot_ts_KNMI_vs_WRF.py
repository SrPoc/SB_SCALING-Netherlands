...
if var_name == 'T':
    ...
    var_name_land = 'M'
    var_name_sea = 'M'
    var_name_WRF_land = 'SWDOWN'
    var_name_WRF_sea = 'SWDOWN'
    var_units = ''
    factor_kelvin_to_celsius = 0
else: 
...
for sim_name in sim_names:
    ...
    #------------
    simple_ts_plot(df_medias_horaria_WRF[2].index, df_medias_horaria_WRF[2], color=colors[2], linestyle = '--', data_label=f'{var_name_WRF_land} (WRF)', ax = ax3, str_title = f'"Center" region', str_ylabel = var_units)
    ...
    plt.savefig(f'{ruta_actual}/figs/ts/Obs-vs-Model/{sim_name}/{var_name}_{sim_name}_{date_of_interest}_AVG-region_comparison.png')
...
simple_ts_plot(df_resultado_WRF_sea.index, df_resultado_WRF_sea - factor_kelvin_to_celsius, color='blue', linestyle = '--', data_label=f'{var_name_WRF_sea} (WRF)', ax = ax2b, str_title = f'"Sea" region (STN = {sea_station_code})', str_ylabel = var_units)
