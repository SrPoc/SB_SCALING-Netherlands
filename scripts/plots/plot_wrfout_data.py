...
def plot_wrf_variable(variable, lats, lons, fig, subplot_idx, cbar_lims):
    ...
    contour = ax.contourf(to_np(lons), to_np(lats), to_np(variable), 10, cmap='coolwarm', transform=ccrs.PlateCarree(), vmin=cbar_lims[0], vmax=cbar_lims[1])
    ...
    cbar = fig.colorbar(contour, ax=ax, orientation='vertical', label=f"{variable.name} ({variable.attrs.get('units', 'No units')})", shrink=0.4, pad=0.02)
...
def plot_wind(variable_u, variable_v, lats, lons, fig, subplot_idx, cbar_lims):
    ...
    contour = ax.contourf(to_np(lons), to_np(lats), ws, 10, cmap='viridis', transform=ccrs.PlateCarree(), vmin=cbar_lims[0], vmax=cbar_lims[1])
    ...
    plt.colorbar(contour, ax=ax, orientation='vertical', label="Wind Speed (m/s)", shrink=0.4, pad=0.02)
    
    # Dibujar los vectores de viento (quiver)
    ax.quiver(to_np(lons[::10, ::10]), to_np(lats[::10, ::10]), u_np[::10, ::10], v_np[::10, ::10], transform=ccrs.PlateCarree())
...
if __name__ == "__main__":
    ...
    file_path = ruta_actual / 'data' / 'Models' / 'WRF' / 'PrelimSim' 

    var_name = "T2"  # Ejemplo: temperatura a 2 metros

    for file_name in sorted(filename for filename in os.listdir(file_path) if filename.startswith("wrfout_d02_2014-07-")):
        ...
        plot_wrf_variable(variable, lats, lons, fig, subplot_idx=121, cbar_lims=(283, 292))
        ...
        plot_wind(u10, v10, lats, lons, fig, subplot_idx=122, cbar_lims=(0, 11))  # 1 fila, 2 columnas, posición 2
    ...
    #     plt.tight_layout()
        ...
        fig.savefig(Path.cwd() / 'figs' / f'T-wind_subplot_{file_name[11:-3]}.png', bbox_inches='tight')

        #CREAR UN GIF DE TODAS LAS IMAGENES GENERADAS
        # Abrimos las imágenes y las convertimos a formato de PIL
        
        path_to_figs = Path.cwd().joinpath('figs')

        images = [Image.open(png) for png in sorted(list(path_to_figs.glob('T-wind_subplot_*.png')))]

        # Guardamos las imágenes en formato GIF
        images[0].save(Path.cwd() / 'figs' / f'T-wind_subplot_{file_name[11:5]}.gif', save_all=True, append_images=images[1:], optimize=False, duration=600, loop=0)
